    ...
    text = ''
      ...
      {        
        if ! nvluks-srv-app -n; then
          printf "Note: Unable to stop nvluks service -> rebooting\n";
        fi
      ...
      if printf "%s" "$defaultManufactureKey" | cryptsetup --test-passphrase luksOpen --key-file=- "$luksDev" >/dev/null 2>&1; then
